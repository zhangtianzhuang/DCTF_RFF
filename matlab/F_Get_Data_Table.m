function [Plot_Table] = F_Get_Data_Table(Plot_Table_Size, Plot_Table_Max, Input_Data, Data_Offset_Length)
    % Plot Table
	Plot_Table = zeros(2*Plot_Table_Size+2,2*Plot_Table_Size+2);
%     Plot_Talbe = zeros(2*Plot_Table_Size,2*Plot_Table_Size);
	...
	if(Process_Length>0)
        for n = Data_Offset_Length+1:Process_Length-Data_Offset_Length
            ...
            Plot_Table(Plot_Table_Size+Q_Index+1,Plot_Table_Size+I_Index+1) = Plot_Table(Plot_Table_Size+Q_Index+1,Plot_Table_Size+I_Index+1)+1;
        end   
    end
