...
% linux config
    dataRoot = '/home/bjtu/ztz/WIFI_Dataset';
end

IS_PLOT = 0;
dataType = 'FilteredAug_DoubleInterval_5_15';
dataSource = [dataRoot, '/WiFi_Data_Waveform_Filtered_Data'];
...
localList = {'P1', 'P2', 'P3'}; %, 'P2', 'P3', 'P4'
frame_header = 30; % 帧头开始位置
...
%                     Plot_Table(:,end-1:end) = 0;
                ...
                [PlotTableRow, PlotTableCol] = size(PlotTableSet{1});
                PlotTable = zeros(PlotTableRow, PlotTableCol);
                for pos = 1:length(PlotTableSet)
                    PlotTable = PlotTable + PlotTableSet{pos};
                end
                ...
                PlotTable(1:2,:) = 0;
                PlotTable(end-1:end,:) = 0;
                PlotTable(:,1:2) = 0;
                PlotTable(:,end-1:end) = 0;
                
                min_table=min(min(PlotTable));  % minmum of the matrix
                max_table=max(max(PlotTable));  % maxmum of the matrix
                PlotTable = PlotTable ./ (max_table-min_table) * 255;
                
                img = graycolor(PlotTable);
