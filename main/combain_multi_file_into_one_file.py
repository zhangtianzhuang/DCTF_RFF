...
if __name__ == '__main__':
    ...
    snr_name = util.buildSNR(SNRs)
    c_name = util.buildC(common_config_device_count)
    ...
    target_file = root + '/' + data_set_type + '_' + util.buildP(P, data_for) + '_' \
                  + c_name + '_' + snr_name + '.txt'
    file_list = list()
    for s in SNRs:
        test_set_file = data_set_type + '_' + util.buildP(P, data_for) + '_' \
                        + c_name + '_' + util.buildSNR([s]) + '.txt'
